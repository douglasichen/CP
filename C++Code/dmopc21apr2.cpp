...
ll ways(vector<ll> Ts, ll P, ll B) {
	...
	ll ans=0;
    if (P) {
        for (int i=2; i<=M+1; i++) sms[i]=fpw(i-1,P);
        int j=M-1;
        for (int i=M+2; i<=2*M; i++) sms[i]=fpw(j,P), j--;
		for (int i : Ts) {
            ans+=sms[i];
            ans%=md;
        }
...
void fl(vector<int> &D, int &a, int &b, int &emp, int &hlf, int &cnt) {
	cnt++;
    if (a&&b) {
        D[a+b]++;
        D[a+b+1]--;
...
int main() {
	...
	for (int x=0,a,b; x<N; x++) {
        a=V[0][x]; b=V[1][x];
		if (x%2==0) fl(dA,a,b,empEven,hlfEven,even);
		else fl(dB,a,b,empOdd,hlfOdd,odd);
	}
